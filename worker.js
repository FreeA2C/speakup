...
.phrase-bar{background:#1a1d27;border-bottom:1px solid #2e3248;padding:10px 20px;display:flex;align-items:center;gap:10px}
.phrase-label{font-size:10px;font-weight:600;color:#4f8ef7;text-transform:uppercase;letter-spacing:.08em;white-space:nowrap}
.phrase-text{font-size:13px;color:#e8eaf0;font-style:italic}
...
<div class="phrase-bar"><span class="phrase-label">Today's phrase</span><span class="phrase-text">"For the first time in a while."</span></div>
<div id="chat">
<div class="welcome"><h2>Hello! Let's practice. 👋</h2><p>話しかけてください。英語でも日本語でもOK。<br>添削と解説は英語＋日本語で表示します。</p>
<div class="hint"><div class="hl">💡 Try saying</div>"I slept well last night for the first time in a while."</div></div>
</div>
<footer>
      ...
      try {
        ...
        const response = await fetch('https://api.anthropic.com/v1/messages', {
          ...
          body: JSON.stringify({
            ...
            max_tokens: 1000,
            system: `You are a friendly English conversation practice assistant for a Japanese adult learner (age 47, technical engineer, intermediate level). ALWAYS respond in this exact JSON format: {"reply": "Your reply","correction": {"needed": true or false,"original": "original if needed","fixed": "fixed if needed","explanation_en": "explanation","explanation_ja": "日本語説明"}}. Be warm, keep replies short (2-3 sentences). Focus on one correction at a time.`,
            messages: messages,
          }),
        });
